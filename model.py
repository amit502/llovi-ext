...
class DeepSeek(Model):
    ...
    def forward(self, head, prompts):
        ...
        for i, prompt in enumerate(prompts):
            ...
            if self.post_process_fn.__name__=='first_char_as_answer':
                response = self.get_response(
                model=self.model_name,
                messages=messages,
                format=AnswerFormat.model_json_schema()
                #temperature=self.temperature,
            ) 
            else:
                response = self.get_response(
            ...
            info = dict() # dict(response.usage)  # completion_tokens, prompt_tokens, total_tokens
            info['response'] = messages[-1]["content"]
            info['message'] = messages
